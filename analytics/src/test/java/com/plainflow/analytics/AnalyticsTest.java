...
@RunWith(BurstJUnit4.class) public class AnalyticsTest {
  ...
  @Test public void enqueueIsDispatched(TestUtils.MessageBuilderTest builder) {
  ...
  @Test public void doesNotEnqueueWhenTransformerReturnsFalse(TestUtils.MessageBuilderTest builder) {
    MessageBuilder messageBuilder = builder.get().userId("prateek");
  ...
  @Test public void doesNotEnqueueWhenInterceptorReturnsNull(TestUtils.MessageBuilderTest builder) {
    MessageBuilder messageBuilder = builder.get().userId("prateek");
    when(messageTransformer.transform(messageBuilder)).thenReturn(true);
