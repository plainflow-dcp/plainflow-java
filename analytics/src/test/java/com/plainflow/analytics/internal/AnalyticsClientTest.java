...
import com.plainflow.analytics.TestUtils;
...
import com.plainflow.analytics.internal.AnalyticsClient.BatchUploadTask;
...
public class AnalyticsClientTest {
  ...
  @Mock
  PlainflowService plainflowService;
  ...
  AnalyticsClient newClient() {
    return new AnalyticsClient(messageQueue, plainflowService, 50, TimeUnit.HOURS.toMillis(1), log,
        threadFactory, networkExecutor, Collections.singletonList(callback));
  }

  @Test public void enqueueAddsToQueue(TestUtils.MessageBuilderTest builder) throws InterruptedException {
  ...
  @Test public void batchRetriesForNetworkErrors() {
    ...
    when(plainflowService.upload(batch)).thenThrow(retrofitError)
    ...
    verify(plainflowService, times(4)).upload(batch);
  ...
  @Test public void batchDoesNotRetryForNonNetworkErrors() {
    ...
    doThrow(retrofitError).when(plainflowService).upload(batch);

    BatchUploadTask batchUploadTask = new BatchUploadTask(client, BACKO, batch);
    batchUploadTask.run();

    // Verify we only tried to upload once.
    verify(plainflowService).upload(batch);
  ...
  @Test public void givesUpAfterMaxRetries() {
    ...
    verify(plainflowService, times(50)).upload(batch);
