...
import com.plainflow.analytics.gson.AutoValueAdapterFactory;
import com.plainflow.analytics.gson.ISO8601DateAdapter;
...
import com.plainflow.analytics.internal.AnalyticsClient;
...
/**
 * The entry point into the Plainflow for Java library.
 * <p>
 * The idea is simple: one pipeline for all your data. Plainflow is the single hub to collect,
 * translate and route your data with the flip of a switch.
 * <p>
 * Analytics for Java will automatically batch events and upload it periodically to Plainflow's
 * servers for you. You only need to instrument Plainflow once, then flip a switch to install
 * new tools.
 * <p>
 * This class is the main entry point into the client API. Use {@link #builder} to construct your
 * own instances.
 *
 * @see <a href="https://Plainflow/">Plainflow</a>
 */
public class Analytics {
   ...
   *
   * @param secretKey Your project secret key available on the Plainflow dashboard.
   */
  public static Builder builder(String secretKey) {
    return new Builder(secretKey);
  }

  /** Enqueue the given message to be uploaded to Plainflow's servers. */
  ...
  public static class Builder {
    private static final Endpoint DEFAULT_ENDPOINT =
        Endpoints.newFixedEndpoint("https://pipe.plainflow.net");
    ...
    Builder(String secretKey) {
      if (secretKey == null || secretKey.trim().length() == 0) {
        throw new NullPointerException("secretKey cannot be null or empty.");
      }
    ...
    /**
     * Set an endpoint that this client should upload events to. Uses {@code https://pipe.plainflow.net}
     * by default.
     */
    ...
    public Analytics build() {
      ...
      RestAdapter restAdapter = new RestAdapter.Builder()
          ...
          .setRequestInterceptor(new RequestInterceptor() {
            @Override public void intercept(RequestFacade request) {
              request.addHeader(AUTHORIZATION_HEADER, Credentials.basic(secretKey, ""));
            }
          })
      ...
      PlainflowService plainflowService = restAdapter.create(PlainflowService.class);

      AnalyticsClient analyticsClient =
          AnalyticsClient.create(plainflowService, flushQueueSize, flushIntervalInMillis, log,
